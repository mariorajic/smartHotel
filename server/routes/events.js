...
module.exports = function (app) {
    ...
    router.get('/events/sos', ensureAuthenticated, (req, res) => {
        db.sequelize.query('select el.*, r.floor_id, el.room_id, fl.number as floor_num, ev.sos, r.name, r.number from event_logs el\n' +
    ...
    router.get('/events/', ensureAuthenticated, (req, res) => {
        db.sequelize.query('select el.*, r.floor_id, el.room_id, ev.code, ev.message, r.name, r.number from event_logs el\n' +
            'inner join events ev on ev.id = el.event_id\n' +
    ...
    router.get('/events/getByFloorId', ensureAuthenticated, (req, res) => {
        db.sequelize.query('select el.*, r.floor_id, el.room_id, r.name, r.number, ev.code, ev.message from event_logs el\n' +
            'inner join events ev on ev.id = el.event_id\n' +

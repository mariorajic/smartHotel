...
module.exports = function (app) {
    ...
    router.post('/bookings/create', (req, res) => {
        ...
        db.booking.create({
            ...
            req.body.params.booking_keys.forEach(function (key, i) {
                db.sequelize.query('select * from insert_key_into_booking_keys(:keyCode, :bookingID)',
                    {
                        replacements: {
                            keyCode: key.code,
                            bookingID: result.id},
    ...
    router.get('/bookings/getByRoomId', ensureAuthenticated, (req, res) => {
        db.sequelize.query('select * from get_bookings_by_room_id(:roomID)',{
            replacements: {
                roomID: req.query.id },
            type: sequelize.QueryTypes.SELECT}).then(function (bookings) {
            if (!bookings) {
                res.json({
                    success: false,
                    data: {
